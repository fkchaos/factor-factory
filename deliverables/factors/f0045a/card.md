...
- 逻辑一句话：隔夜-日内分解：20 日隔夜累计 − 20 日日内累计。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0134 | 0.140 | 55.2% | 1575 |
| hs800 | 0.0193 | 0.211 | 58.0% | 1575 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor overnight_intraday_decomp --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0045a，供策略组阶段 0 消费）

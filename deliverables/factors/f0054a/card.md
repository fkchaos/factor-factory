...
- 逻辑一句话：超额收益信息比率：市场中性超额收益 60 日均值/标准差。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0083 | -0.061 | 48.3% | 1535 |
| hs800 | -0.0135 | -0.100 | 46.8% | 1535 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor excess_return_ir --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0054a，供策略组阶段 0 消费）

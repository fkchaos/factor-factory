...
- 逻辑一句话：隔夜跳空-波动扩张价差：z(60日隔夜均值) − z(波动扩张)。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0068 | 0.097 | 52.3% | 1555 |
| hs800 | 0.0097 | 0.170 | 57.6% | 1555 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor overnight_gap_volexp_spread --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0048a，供策略组阶段 0 消费）

...
- 逻辑一句话：dispersion_agent
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0003 | -0.005 | 50.6% | 1518 |
| hs800 | -0.0013 | -0.029 | 49.9% | 1518 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor dispersion_agent --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0042a，供策略组阶段 0 消费）

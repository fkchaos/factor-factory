...
- 逻辑一句话：20 交易日动量（跳过最近 1 日，规避 1 日微观结构污染）。
...
|---|---|---|---|---|
| hs300 | -0.0077 | -0.061 | 48.2% | 1574 |
| hs800 | -0.0129 | -0.107 | 45.2% | 1574 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor momentum_20 --pool hs300
- 已知陷阱：特定牛熊阶段 / 流动性枯竭。

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0006a，供策略组阶段 0 消费）

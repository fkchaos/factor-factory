...
- 逻辑一句话：超跌反弹分：z(60日最大回撤深度) × z(5日反弹幅度)。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0006 | -0.008 | 47.3% | 1566 |
| hs800 | 0.0021 | 0.037 | 50.6% | 1566 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor oversold_rebound_score --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0057a，供策略组阶段 0 消费）

...
- 逻辑一句话：volume_expansion_speed
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 1 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0057 | -0.066 | 48.4% | 1476 |
...
- 冗余关系：见 correlation.csv（ρ≥0.7 勿重复入模）；该矩阵在 **none** 单池上计算，非六池平均，换池后相关性可能变化。
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor volume_expansion_speed --pool hs300
- 已知陷阱：特定牛熊阶段 / 流动性枯竭。

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0005a，供策略组阶段 0 消费）

...
- 逻辑一句话：特质收益下尾 beta：个股 idio 对市场-wide 特质下尾变化的 250 日滚动 beta。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0020 | 0.016 | 52.3% | 1344 |
| hs800 | -0.0005 | -0.004 | 49.7% | 1344 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor idio_tail_beta --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0051a，供策略组阶段 0 消费）

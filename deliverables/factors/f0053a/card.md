# rejoicing-regret度（f0053a）

- f-code：f0053a
- 类别 / 方向：正向（因子值越大，未来收益越高）
- 逻辑一句话：rejoicing-regret 度：上涨日 rejoicing 减下跌日 regret，取负号。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0115 | 0.094 | 53.7% | 1575 |
| hs800 | 0.0172 | 0.145 | 56.0% | 1575 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor rejoicing_regret --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0053a，供策略组阶段 0 消费）

...
- 逻辑一句话：第二阶段突破强度（趋势 + 基底突破复合）。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0002 | 0.001 | 52.0% | 1345 |
| hs800 | -0.0041 | -0.029 | 49.5% | 1345 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor stage2_breakout --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0087a，供策略组阶段 0 消费）

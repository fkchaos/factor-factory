...
- 逻辑一句话：隔夜跳空因子：open / 前收 - 1。正值=隔夜跳空高开。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0067 | 0.082 | 51.8% | 1594 |
| hs800 | 0.0075 | 0.106 | 54.0% | 1594 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor overnight_gap --pool hs300
- 已知陷阱：特定牛熊阶段 / 流动性枯竭。

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0008a，供策略组阶段 0 消费）

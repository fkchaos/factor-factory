...
- 逻辑一句话：组内换手异常超出幅度：max(turnover − 组内80%分位, 0) 的 21 日均值。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0103 | -0.112 | 44.1% | 1584 |
| hs800 | -0.0154 | -0.180 | 43.0% | 1584 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor turnover_excess_group --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0059a，供策略组阶段 0 消费）

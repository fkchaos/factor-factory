...
- 逻辑一句话：cashflow_update
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0000 | -0.001 | 50.1% | 1530 |
| hs800 | 0.0016 | 0.031 | 52.1% | 1533 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor cashflow_update --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0086a，供策略组阶段 0 消费）

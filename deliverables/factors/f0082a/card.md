...
- 逻辑一句话：growth_convexity
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0040 | -0.049 | 48.2% | 1569 |
| hs800 | -0.0007 | -0.010 | 48.4% | 1573 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor growth_convexity --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0082a，供策略组阶段 0 消费）

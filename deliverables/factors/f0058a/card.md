...
- 逻辑一句话：beta×q 交互项：60 日市场 beta × 20 日换手率变异系数。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0093 | -0.090 | 45.1% | 1565 |
| hs800 | -0.0115 | -0.114 | 43.5% | 1565 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor beta_qvol_interact --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0058a，供策略组阶段 0 消费）

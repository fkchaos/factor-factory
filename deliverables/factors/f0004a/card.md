...
- 逻辑一句话：chip_cost_distance
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 1 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0058 | -0.053 | 48.3% | 1595 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor chip_cost_distance --pool hs300
- 已知陷阱：特定牛熊阶段 / 流动性枯竭。

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0004a，供策略组阶段 0 消费）

...
- 逻辑一句话：相对市场收益偏离：5 日累计(r_i − r_mkt)。正值 = 持续跑赢。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0172 | -0.154 | 44.3% | 1590 |
| hs800 | -0.0196 | -0.174 | 42.1% | 1590 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor return_deviation_mkt --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0047a，供策略组阶段 0 消费）

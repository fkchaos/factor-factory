...
- 逻辑一句话：隔夜收益占比：60 日隔夜累计 / 60 日总累计。正值 = 收益靠隔夜跳空。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0032 | 0.054 | 51.9% | 1535 |
| hs800 | 0.0035 | 0.076 | 53.5% | 1535 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor overnight_ratio_60d --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0044a，供策略组阶段 0 消费）

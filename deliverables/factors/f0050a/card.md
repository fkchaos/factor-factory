...
- 逻辑一句话：非对称 5 日价格反转：−(close[t]/close[t-5] − 1)。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0170 | 0.151 | 55.4% | 1590 |
| hs800 | 0.0193 | 0.171 | 57.8% | 1590 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor asymmetric_5d_reversal --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0050a，供策略组阶段 0 消费）

...
- 逻辑一句话：SUE = (当季净利 − 去年同季净利) / 过去 8 季该惊喜的标准差。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0055 | 0.068 | 53.2% | 1595 |
| hs800 | 0.0082 | 0.112 | 56.0% | 1595 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor sue --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0076a，供策略组阶段 0 消费）

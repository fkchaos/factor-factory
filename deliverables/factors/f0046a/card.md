...
- 逻辑一句话：特质波动率比率：20 日特质波动 / 120 日特质波动。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0053 | -0.063 | 47.9% | 1475 |
| hs800 | -0.0097 | -0.129 | 44.6% | 1475 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor idio_vol_ratio --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0046a，供策略组阶段 0 消费）

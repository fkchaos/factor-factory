...
- 逻辑一句话：60 日市场 beta（BAB 方向：取负号）。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0078 | 0.058 | 53.7% | 1565 |
| hs800 | 0.0075 | 0.054 | 52.3% | 1565 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor beta_market_60d --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0083a，供策略组阶段 0 消费）

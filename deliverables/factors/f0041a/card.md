...
- 逻辑一句话：lowprice_volume_spike
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0131 | -0.085 | 46.0% | 1491 |
| hs800 | -0.0091 | -0.079 | 44.7% | 1516 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor lowprice_volume_spike --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0041a，供策略组阶段 0 消费）

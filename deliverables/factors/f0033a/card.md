...
- 逻辑一句话：liquidity_improvement
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0060 | 0.080 | 53.9% | 1555 |
| hs800 | 0.0080 | 0.114 | 55.1% | 1555 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor liquidity_improvement --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0033a，供策略组阶段 0 消费）

...
- 逻辑一句话：12-1 动量：close[t-21] / close[t-252] - 1。正值 = 过去约 11 个月上涨的票。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0070 | 0.051 | 54.1% | 1343 |
| hs800 | 0.0045 | 0.033 | 51.8% | 1343 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor momentum_12_1 --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0035a，供策略组阶段 0 消费）

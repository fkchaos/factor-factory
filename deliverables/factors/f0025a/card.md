...
- 逻辑一句话：bollinger_upper_20d
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 1 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | -0.0114 | -0.094 | 46.2% | 1576 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor bollinger_upper_20d --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0025a，供策略组阶段 0 消费）

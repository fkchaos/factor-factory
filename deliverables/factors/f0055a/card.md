...
- 逻辑一句话：量价背离：价格动量 z 减量比 z，取负号。
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0019 | 0.019 | 50.4% | 1575 |
| hs800 | 0.0003 | 0.003 | 49.5% | 1575 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor price_volume_divergence --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0055a，供策略组阶段 0 消费）

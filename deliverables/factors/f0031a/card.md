# 20日Amihud非流动性（f0031a）

- f-code：f0031a
- 类别 / 方向：正向（因子值越大，未来收益越高）
- 逻辑一句话：amihud_illiquidity_20d
- 是否组合：否
## 分池 IC（全样本，决策5：覆盖 2 池）
| 池 | RankIC | ICIR | IC胜率 | 样本日 |
|---|---|---|---|---|
| hs300 | 0.0097 | 0.128 | 55.8% | 1575 |
| hs800 | 0.0128 | 0.167 | 55.7% | 1575 |
...
- 复现命令：FF_PROVIDER=baostock python scripts/real_research.py --factor amihud_illiquidity_20d --pool hs300

## 聚合视图（本因子在聚合交付中的位置）
- 机器可读发货形态：`../strategy_export/stock_factors.json`（条目 name=f0031a，供策略组阶段 0 消费）
